...
SignalHandler &SignalHandler::getInstance() { return instance; }
...
SignalHandler &SignalHandler::removeListener(int signalCode,
                                             Listener *listener) {
    std::vector<Listener *> &listenerList = listeners[signalCode];

    if (!listenerList
             .empty()) { // if listenerList is already empty then we don't
                         // need to do anything
...
void SignalHandler::platform_registerSignal(int signalCode) {
    std::cout << "Signalhandler is not yet implemented on Windows."
              << std::endl;
}
