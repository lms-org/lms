...
MasterServer::Client::Client(int fd, const std::string &peer) : fd(fd), reader(fd), writer(fd), peer(peer) {}
...
void MasterServer::processClient(Client &client, const std::string &message) {
    if(message == "clients") {
        ...
        client.writer.writeLine(std::string("TCPIP Server at ") + std::to_string(err));
    } else if(message == "run") {
...
void connectToMaster(int argc, char *argv[]) {
    ...
    TCLAP::SwitchArg pidSwitch("", "pid", "Get process ID of master server", cmd);
    TCLAP::SwitchArg infoSwitch("", "info", "Get version of master server", cmd);
    TCLAP::SwitchArg connSwitch("", "clients", "Get all clients connected to master server", cmd);
    TCLAP::SwitchArg shutdownSwitch("", "shutdown", "Shutdown master server", cmd);
    TCLAP::ValueArg<std::string> runArg("r", "run", "Start a runtime with config", false, "", "configfile", cmd);
    cmd.parse(argc, argv);

    lms::internal::MasterClient client = lms::internal::MasterClient::fromUnix("/tmp/lms.sock");
