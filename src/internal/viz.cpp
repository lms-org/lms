...
void dumpModuleChannelGraph(ModuleChannelGraph<Module*>& graph, DotExporter &dot, const std::string &prefix) {
    ...
    for (const auto &ch : graph) {
        ...
        for (const auto &access : ch.second) {
            ...
            if(access.permission == ModuleChannelGraph<Module*>::Permission::WRITE) {
                dot.edge(prefix + "_" + access.module->getName(), prefix + "_" + ch.first);
            } else {
                dot.edge(prefix + "_" + ch.first, prefix + "_" + access.module->getName());
            }
            dot.reset();
