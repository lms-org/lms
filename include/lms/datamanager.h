...
struct Any final {
    void operator* () {
        LMS_EXCEPTION("Cannot call operator* for lms::Any: Any is afraid of stars");
    }

    void* operator-> () {
        LMS_EXCEPTION("Cannot call operator-> for lms::Any: Robin Hood won't give you his arrow");
    }
};

template <typename Source, typename Target, bool condition>
struct CastIf;
...
struct Object : public FakeObject<T> {
    ...
    Inheritance *getInheritance() override {
        return CastIf<T,Inheritance,std::is_base_of<Inheritance,T>::value>::call(&this->value);
    }

    Serializable* getSerializable() override {
        return CastIf<T,Serializable,std::is_base_of<Serializable,T>::value>::call(&this->value);
    }
};
...
protected:
    ...
    struct InheritanceCallerGet<A, false> {
        static A* call(DataChannel<T> *obj) {
            if(obj->m_internal->main->supportsInheritance()) {
                return dynamic_cast<A*>(obj->m_internal->main->getInheritance());
            } else {
    ...
    struct InheritanceCallerGet<A, true> {
        static A* call (DataChannel<T> *obj) {
            return static_cast<A*>(obj->m_internal->main->get());//avoid casting to void*
        }
    };
    ...
    T* get_() {
        ...
        return InheritanceCallerGet<T,std::is_fundamental<T>::value>::call(this);
...
public:
    ...
    std::shared_ptr<DataChannelInternal> accessChannel(const std::string &name) {
        ...
        if (!channel) {
            ...
            if (!channel->main) {
                channel->main.reset(new Object<T>());
                LMS_EXCEPTION("Invalid state: channel is initialized but object is null");
            } else {
                TypeResult typeRes = channel->main->checkType<T>();
                if (typeRes == TypeResult::INVALID) {
                    LMS_EXCEPTION("Tried to access channel " + name + " (" + channel->main->typeName()
                        + ") as " + lms::typeName<T>());
